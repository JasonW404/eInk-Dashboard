...
@patch("inkpi.network.privileged.subprocess.run", return_value=_ok_result("SSID:TestNet\n"))
...
def test_wifi_connect_passes_password_via_stdin(mock_run: MagicMock) -> None:
    ...
    assert "secret123" not in str(connect_call.args[0])


@patch("inkpi.network.privileged.subprocess.run", return_value=_ok_result())
def test_hotspot_password_passes_only_via_stdin(mock_run: MagicMock) -> None:
    result = handle_command(
        {
...
@patch("inkpi.network.privileged.subprocess.run", return_value=_ok_result())
def test_legacy_hotspot_rotate_password_fails_safely(mock_run: MagicMock) -> None:
    result = handle_command(
        {
    ...
    assert "hotspot_configure" in result["message"]
    mock_run.assert_not_called()


@patch("inkpi.network.privileged.subprocess.run", return_value=_ok_result("device status\n"))
...
@patch("inkpi.network.privileged.subprocess.run", return_value=_fail_result("connection failed"))
...
@patch("inkpi.network.privileged.subprocess.run")
