...
class EPDAdapter:
    ...
    def __init__(self, width: int = 800, height: int = 480, orientation: str = "landscape") -> None:
        ...
        try:
            from inkpi.hardware.waveshare_epd import epd4in26
    ...
    def display(self, image: Image.Image, mode: RefreshMode = RefreshMode.FULL) -> bool:
        ...
        if image.size != (self._width, self._height):
            self._logger.error(
                f"Image size mismatch: expected {self._width}x{self._height}, got {image.width}x{image.height}"
            )
            return False
        ...
        if not self._hardware_available:
            self._logger.info(f"Simulating EPD display (mode={mode.value}, size={image.width}x{image.height})")
            return True
    ...
    def _prepare_for_mode(self, mode: RefreshMode) -> None:
        ...
        if mode == RefreshMode.FULL and self._last_refresh_mode == RefreshMode.PARTIAL:
            self._logger.info("Reinitializing EPD before full refresh after partial mode")
