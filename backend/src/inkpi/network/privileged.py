...
from inkpi.network.network_helper import CommandStep, plan_network_operation
...
def handle_command(request: dict[str, Any]) -> dict[str, Any]:
    ...
    if op_request.action == "hotspot_rotate_password":
        return {
            "operation_id": operation_id,
            "status": "failed",
    ...
    password = op_request.password or ""
