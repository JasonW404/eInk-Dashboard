...
class NetworkHelper(Protocol):
    """Narrow privileged-helper boundary consumed by `inkpi-api`."""
...
class InMemoryNetworkHelper:
    ...
    operations. Transient passwords are never copied into tracked operations.
    """
...
def build_operation_request(
    ...
    if action == "wifi_connect":
        ...
        password = _optional_secret(payload, "password", min_length=1)
    ...
    if action in {"hotspot_enable", "hotspot_configure"}:
...
def _optional_secret(
    ...
    *,
    ...
    if value is None or value == "":
        return None
    if not isinstance(value, str) or len(value) < min_length or len(value) > 63:
        raise ValueError(f"{key} must contain {min_length} to 63 characters")
    return value
...
def _message_for(request: NetworkOperationRequest) -> str:
    ...
    if request.action == "hotspot_configure":
        return "Hotspot configuration queued"
