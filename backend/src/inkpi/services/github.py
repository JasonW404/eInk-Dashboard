...
class GitHubService:
    ...
    def get_monthly_stats(self) -> GitHubMonthlyStats:
        ...
        contribution_stats = self._fetch_username_contributions(month_start, now)
        if contribution_stats is not None:
            self._cached_monthly_stats = contribution_stats
        ...
        if not self._api.has_token():
            self._logger.warning("github_api_key_missing private_repo_data_will_not_be_included")
        else:
        ...
        stats = GitHubMonthlyStats(
            ...
            pull_requests=self._fetch_pull_request_count(month_start),
        )
    ...
    def _fetch_username_contributions(
        self,
        month_start: date,
        now: datetime,
    ) -> GitHubMonthlyStats | None:
        """Use GitHub's contribution collection as the canonical user scope."""

        fetch = getattr(self._api, "fetch_user_contributions", None)
        if fetch is None or not self._username or not self._api.has_token():
            return None
        try:
            payload = fetch(
                self._username,
                datetime.combine(month_start, datetime.min.time(), tzinfo=UTC).isoformat(),
                now.isoformat(),
            )
        except Exception:
            self._logger.exception("github_contribution_collection_failed")
            return None
        if not isinstance(payload, dict):
            return None

        daily: Counter[date] = Counter()
        calendar = payload.get("contributionCalendar")
        ...
        for week in weeks if isinstance(weeks, list) else []:
            ...
            for item in week.get("contributionDays", []):
                ...
                try:
                    ...
                    count = max(0, int(item.get("contributionCount", 0)))
        ...
        return GitHubMonthlyStats(
            ...
            contributions=[GitHubContributionDay(day=day, commit_count=value) for day, value in sorted(daily.items())],
            user_monthly_commit_count=count("totalCommitContributions"),
            user_monthly_code_lines=0,
            organization_user_monthly_commit_count=0,
            organization_user_monthly_code_lines=0,
            pull_requests=count("totalPullRequestContributions"),
        )

    def _fetch_pull_request_count(self, month_start: date) -> int:
        if not self._username:
            return 0
        fetch = getattr(self._api, "fetch_user_pull_request_count", None)
        if fetch is None:
            return 0
        try:
            return int(fetch(self._username, month_start.isoformat()))
        except Exception:
            self._logger.exception("github_pull_request_count_failed")
            return 0
